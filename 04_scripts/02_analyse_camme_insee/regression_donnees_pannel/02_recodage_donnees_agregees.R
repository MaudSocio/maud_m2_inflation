...
df <- fread("C:/Users/ymaud/dev/inflation_memoire/02_data/agregees/Inflation_base_all_years.csv")
...
## Recodage de df$CLASPRO2 en PROFESSION_ap_2017 
df$PROFESSION_ap_2017 <- df$CLASPRO2 %>%
  as.character() %>%
  fct_recode(
    ...
    "Employé, profession intermédiaire" = "6",
    "Ingénieur, cadre, directeur" = "7"
  )

### Recodage de df$CLASSPRO en PROFESSION_av_2017 
df$PROFESSION_av_2017 <- df$CLASSPRO %>%
  as.character() %>%
  fct_recode(
    NULL = "0",
    "Ouvrier, technicien, agent de maîtrise" = "1",
    ...
    "Ouvrier, technicien, agent de maîtrise" = "3",
    "Ouvrier, technicien, agent de maîtrise" = "4",
    "Ouvrier, technicien, agent de maîtrise" = "5",
    "Employé, profession intermédiaire" = "6",
    "Ingénieur, cadre, directeur" = "7",
    "Ingénieur, cadre, directeur" = "8",
    "Employé, profession intermédiaire" = "9"
  )


# Fusionner les deux recodages dans une seule colonne 'PROFESSION'
library(dplyr)
library(forcats)

# Convertir les colonnes en facteurs si ce n'est pas déjà fait
df$PROFESSION_av_2017 <- factor(df$PROFESSION_av_2017)
df$PROFESSION_ap_2017 <- factor(df$PROFESSION_ap_2017)

# Créer une nouvelle colonne 'PROFESSION' en choisissant les données en fonction de l'année
df <- df %>%
  mutate(PROFESSION = ifelse(ANENQ >= 2017, as.character(PROFESSION_ap_2017), as.character(PROFESSION_av_2017)))

# Convertir 'PROFESSION' en facteur après fusion
...
write.csv(df,"C:/Users/ymaud/dev/inflation_memoire/02_data/agregees/Inflation_base_all_years_v3.csv", row.names = FALSE)
